...
class PortfolioForm(forms.ModelForm):
    ...
    def clean_name(self):
        ...
        if Portfolio.objects.filter(user=user, name=name).exclude(
            ...
            raise forms.ValidationError(
                f'A portfolio with the name "{name}"'
    ...
    def __init__(self, *args, **kwargs):
        """
        Initialize the form and set the color field to a
...
class AssetForm(forms.ModelForm):
    ...
    def clean_name(self):
        ...
        if Asset.objects.filter(portfolio=portfolio, name=name).exclude(
                pk=self.instance.pk).exists():
            raise forms.ValidationError(
                f'An asset with the name "{name}" already'
                f' exists in this portfolio.'
                )
...
class TransactionForm(forms.ModelForm):
    ...
    asset_choice = forms.ChoiceField(
        ...
        required=False,
        help_text=(
            "Choose an existing asset or select 'NEW ASSET' "
            "to create a new one."
        )
    )
    new_asset_name = forms.CharField(
        ...
        help_text=(
            "Enter the name of the new asset (Max length: 8 characters)."
                ),
    ...
    def clean_new_asset_name(self):
        ...
        if new_asset_name and len(new_asset_name) > 8:
            raise forms.ValidationError(
                "Asset name cannot exceed 8 characters."
                )
    ...
    def __init__(self, *args, **kwargs):
        ...
        Set the initial asset choice and default values for
        action and type fields.
        """
        ...
        if portfolio:
            ...
            asset_choices.insert(1, ('new', 'NEW ASSET'))
    ...
    def clean(self):
        """
        Validate the form to ensure the new asset name is provided if
        'NEW ASSET' is selected.
        """
