...
class ContactForm(forms.ModelForm):
    ...
    class Meta:
        model = Contact
        fields = ['name', 'email', 'message']  # Fields to be displayed in the form

    def __init__(self, *args, **kwargs):
        """
        Initialize the form and capture the request object if passed.
        The request object is needed to send the appropriate signal with context.
        """
        self.request = kwargs.pop('request', None)  # Pop 'request' from kwargs
        super(ContactForm, self).__init__(*args, **kwargs)  # Initialize parent class
    ...
    def save(self, *args, **kwargs):
        ...
        Raises:
            Exception: If there is an error during the save process, the error is raised again.
        """
        try:
            ...
            if self.request:
                FormSignals.form_submission_success.send(
                    ...
                    request=self.request    # The current request (passed to the signal handler)
                )
