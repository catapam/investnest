...
class UpdateUsernameView(FormView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        Checks if the form is valid. If valid, calls form_valid(), otherwise form_invalid().
        """
    ...
    def get_form(self, form_class=None):
        """
        Override the get_form method to bind the form to the current user instance.
        This ensures that the form updates the logged-in user's username.
        """
        if self.request.method == 'POST':
            # Bind the form to POST data and the current user instance during POST requests
            return self.form_class(self.request.POST, instance=self.request.user)
        # Bind the form only to the current user instance for GET requests (prepopulate data)
    ...
    def form_valid(self, form):
        ...
        # Add a success message to display to the user after updating their username
        messages.success(self.request, 'Your username has been updated successfully!')
        return super().form_valid(form)

    def form_invalid(self, form):
        """
        Handle invalid form submissions. Display an error message and re-render the form.
        """
